...
class Limb(part.Part):
    ...
    def build(self):
        ...
        rigrepo.libs.attribute.lockAndHide(paramNode, 
        ...
        mc.addAttr(paramNode, ln="ikfk_switch", nn="Snap IK FK", 
                at="enum", keyable=False, enumName="IK:FK")
        mc.setAttr("{}.ikfk_switch".format(paramNode), cb=True)  

        #connect the param ikfk attr to the ikfk system group ikfk attribute
        mc.connectAttr("{}.ikfk".format(paramNode), 
                "{}.ikfk".format(grp), f=True)
        ...
        for jnt, blendJnt in zip(self.jointList, blendJointList):
            ...
            mc.connectAttr("{}.s".format(blendJnt), "{}.s".format(jnt), f=True)

        for jnt, fkCtrl in zip(fkJointList,fkControlNames):
            ...
            if fkCtrl == fkControlNames[-1]:
                ...
                fkGimbalCtrl = rigrepo.libs.control.create(name=fkCtrl.replace("_{}".format(side),
                                                                 "_gimbal_{}".format(side)), 
                                                            controlType="sphere",
                                                            hierarchy=[],
                                                            transformType="joint",
                                                            hideAttrs=["tx", "ty", "tz","v"],
        ...
        mc.setAttr("{}.preferredAngle".format(self._fkControls[1]), 
                        *mc.getAttr("{}.preferredAngle".format(self.jointList[1]))[0])

        # create the polevector control
        poleVectorPos = rigrepo.libs.ikfk.IKFKLimb.getPoleVectorFromHandle(handle, ikJointList)
        ...
        ikGimbalCtrl = rigrepo.libs.control.create(name=ikControlNames[1].replace("_{}".format(side), 
                                                            "_gimbal_{}".format(side)), 
                                                controlType="sphere",
                                                hierarchy=[],
                                                position=endJointPos,
        ...
        mc.delete(mc.aimConstraint(tempJnt, ikCtrl, wut="object", wuo=tempUpJnt,  
        ...
        blendNode = mc.ls(mc.listConnections(ikJointList[1], destination=False,source=True,scn=True),type="blendColors")[0]
        ...
        upperDistanceBetweenNode = mc.createNode('distanceBetween', 
                                                    n="{}_upperPvPin_dst".format(self.name))
        ...
        mc.connectAttr("{}.worldMatrix[0]".format(upperDistJnt), 
                        "{}.inputMatrix".format(upperLimbDecomp), f=True)
        mc.connectAttr("{}.worldMatrix[0]".format(pvCtrl), 
                        "{}.inputMatrix".format(pvDecomp), f=True)
        mc.connectAttr("{}.outputTranslate".format(upperLimbDecomp), 
                        "{}.point1".format(upperDistanceBetweenNode), f=True)
        mc.connectAttr("{}.outputTranslate".format(pvDecomp), 
                        "{}.point2".format(upperDistanceBetweenNode), f=True)

        lowerLimbDecomp = mc.createNode('decomposeMatrix', n="{}_lowerPvPin_dcm".format(self.name))
        lowerDistanceBetweenNode = mc.createNode('distanceBetween', 
                        n="{}_lowerPvPin_dst".format(self.name))
        mc.connectAttr("{}.worldMatrix[0]".format(ikCtrl), 
                        "{}.inputMatrix".format(lowerLimbDecomp), f=True)
        mc.connectAttr("{}.outputTranslate".format(lowerLimbDecomp), 
                        "{}.point1".format(lowerDistanceBetweenNode), f=True)
        mc.connectAttr("{}.outputTranslate".format(pvDecomp), 
                        "{}.point2".format(lowerDistanceBetweenNode), f=True)
        ...
        mc.connectAttr("{}.outputR".format(blendNode), 
                        "{}.input1X".format(multDivideNormalize), f=True)
        mc.connectAttr("{}.outputG".format(blendNode), 
                        "{}.input1Y".format(multDivideNormalize), f=True)

        # set the attributes to normalize the value before it goes into the blend colors node.
        mc.setAttr("{}.input2X".format(multDivideNormalize), 
                    mc.getAttr("{}.input1X".format(multDivideNormalize)))
        mc.setAttr("{}.input2Y".format(multDivideNormalize), 
                    mc.getAttr("{}.input1Y".format(multDivideNormalize)))
        ...
        mc.connectAttr("{}.outputX".format(multDivideNormalize), 
                        "{}.color2R".format(pvPinBlendNode), f=True)
        mc.connectAttr("{}.outputY".format(multDivideNormalize), 
                        "{}.color2G".format(pvPinBlendNode), f=True)
        ...
        mc.disconnectAttr("{}.outputR".format(blendNode), 
                            "{}.t{}".format(ikJointList[1], aimAttr.strip("-")))
        mc.disconnectAttr("{}.outputG".format(blendNode), 
                            "{}.t{}".format(ikJointList[2], aimAttr.strip("-")))

        # make sure if it is a negative direction that we account for that and invert the values
        # coming into the pinning blend node.
        multDivideDistanceNormalize = mc.createNode("multiplyDivide", 
                                                    n="{}_pvDistance_mdn".format(self.name))
        mc.setAttr("{}.operation".format(multDivideDistanceNormalize), 2)
        if "-" in aimAttr:
            pvPinUpperRvrMultDouble = mc.createNode("multDoubleLinear", 
                                                    n="{}_PvPinUpperRvr_mdl".format(self.name))
            pvPinLowerRvrMultDouble = mc.createNode("multDoubleLinear", 
                                                    n="{}_PvPinLowerRvr_mdl".format(self.name))
            mc.connectAttr("{}.distance".format(upperDistanceBetweenNode), 
                            "{}.input1".format(pvPinUpperRvrMultDouble), f=True)
            mc.connectAttr("{}.distance".format(lowerDistanceBetweenNode), 
                            "{}.input1".format(pvPinLowerRvrMultDouble), f=True)

            for node in (pvPinLowerRvrMultDouble, pvPinUpperRvrMultDouble):
                mc.setAttr("{}.input2".format(node), -1)

            mc.connectAttr("{}.output".format(pvPinUpperRvrMultDouble), 
                            "{}.input1X".format(multDivideDistanceNormalize), f=True)
            mc.connectAttr("{}.output".format(pvPinLowerRvrMultDouble), 
                            "{}.input1Y".format(multDivideDistanceNormalize), f=True)
        else:
            mc.connectAttr("{}.distance".format(upperDistanceBetweenNode), 
                            "{}.input1X".format(multDivideDistanceNormalize), f=True)
            mc.connectAttr("{}.distance".format(lowerDistanceBetweenNode), 
        ...
        mc.connectAttr("{}.outputX".format(multDivideDistanceNormalize), 
                        "{}.color1R".format(pvPinBlendNode), f=True)
        mc.connectAttr("{}.outputY".format(multDivideDistanceNormalize), 
                        "{}.color1G".format(pvPinBlendNode), f=True)
        # set the attributes to normalize the value before it goes into the blend colors node.
        mc.setAttr("{}.input2X".format(multDivideDistanceNormalize), 
                    mc.getAttr("{}.input1X".format(multDivideNormalize)))
        mc.setAttr("{}.input2Y".format(multDivideDistanceNormalize), 
                    mc.getAttr("{}.input1Y".format(multDivideNormalize)))
        ...
        mc.connectAttr("{}.outputX".format(reverseNode), 
                        "{}.input2".format(pvPinMultDouble), f=True)
        ...
        if mc.objExists(anchor):
            ...
            mc.xform(anchorGrp, ws=True, matrix=mc.xform(self.jointList[0], 
                                                            q=True, ws=True, matrix=True))
        ...
        if createBendyLimb:
            ...
            mc.skinPercent(bindMeshSkin , '{}.vtx[0:3]'.format(bindmeshGeometry), 
                            transformValue=[(self.jointList[0], 1.0), 
                            (self.jointList[1], 0.0), (self.jointList[2],0.0)])
            mc.skinPercent(bindMeshSkin , '{}.vtx[4:7]'.format(bindmeshGeometry),  
                            transformValue=[(self.jointList[0], 0.5), 
                            (self.jointList[1], 0.5), (self.jointList[2],0.0)])
            mc.skinPercent(bindMeshSkin , '{}.vtx[8:11]'.format(bindmeshGeometry), 
                            transformValue=[(self.jointList[0], 0.0), 
                            (self.jointList[1], 1.0), (self.jointList[2],0.0)])
            mc.skinPercent(bindMeshSkin , '{}.vtx[12:15]'.format(bindmeshGeometry), 
                            transformValue=[(self.jointList[0], 0.0), 
                            (self.jointList[1], 0.5), (self.jointList[2],0.5)])
            mc.skinPercent(bindMeshSkin , '{}.vtx[16:19]'.format(bindmeshGeometry), 
                            transformValue=[(self.jointList[0], 0.0), 
                            (self.jointList[1], 0.0), (self.jointList[2],1.0)])
            ...
            mc.pointConstraint(self.jointList[0],self.jointList[1], 
                                controlHieracrchyList[1][0], mo=True)
            ...
            mc.pointConstraint(self.jointList[1],self.jointList[2], 
                                controlHieracrchyList[3][0], mo=True)
    ...
    def postBuild(self):
        ...
        if mc.objExists(noTwist):
            mc.aimConstraint(target, noTwist, mo=1, weight=1, 
        ...
        if mc.objExists(noTwist):
            mc.aimConstraint(target, elbowTwistJoint, mo=1, weight=1, 
                                aimVector=aimVector, upVector=(0, 0, 0), worldUpType='none')
        else:
    ...
    def switch(paramNode, value):
        '''
        .. Warning::
            This is not how we're currently handling the switch in the rigs. It's not up
            to date and shouldn't be used until it's updated.
        '''
