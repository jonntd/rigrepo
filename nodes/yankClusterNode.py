...
class YankClusterNode(commandNode.CommandNode):
    ...
    def __init__(self, name, parent=None, transforms='mc.ls(type="transform")', clusters='mc.ls(type="cluster")',
                    selected=True, geometry="body_geo"):
        super(YankClusterNode, self).__init__(name, parent)
        commandAttribute = self.getAttributeByName('command')
        self.addAttribute('transforms', transforms, attrType=str, index=0)
        self.addAttribute('clusters', clusters, attrType=str, index=1)
        self.addAttribute('selected', selected, attrType=bool, index=2)
        self.addAttribute('geometry', geometry, attrType=str, index=2)
        cmd = '''
import maya.cmds as mc
import traceback
...
try:
    ...
    if {selected}:
        ...
        selectionList = mc.ls("{geometry}", dag=1, s=1, ni=1)
...
'''
    ...
    def execute(self, *args, **kwargs):
        ...
        selected = self.getAttributeByName("selected").getValue()
